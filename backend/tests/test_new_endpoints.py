...
import app as app_module  # noqa: E402  module-level in-memory stores live here
...
def clean_state(app):
    """Reset DB rows AND the module-level in-memory stores between tests."""
...
SAMPLE_PATIENT = {
    "Age": 78, "Gender": 0, "AppointmentLeadTimeDays": 21,
    "SMSReceived": 0, "PriorDNACount": 4, "IMDDecile": 2, "Disability": 1,
}
...
class TestCarerProxy:
    ...
    def test_create_success(self, client):
        token = login(client)
        res = client.post("/api/carer-proxy", headers=auth(token), json={
            "carerName": "Jane Doe", "relationship": "family",
            "patientIdentifier": "NHS001", "carerContact": "07700900000",
    ...
    def test_invalid_relationship(self, client):
        token = login(client)
        res = client.post("/api/carer-proxy", headers=auth(token), json={
            "carerName": "Jane Doe", "relationship": "stranger",
    ...
    def test_list_scoped_to_user(self, client):
        token = login(client)
        client.post("/api/carer-proxy", headers=auth(token), json={
            "carerName": "Jane Doe", "relationship": "carer",
    ...
    def test_create_writes_audit_entry(self, client, app):
        token = login(client)
        client.post("/api/carer-proxy", headers=auth(token), json={
            "carerName": "Jane Doe", "relationship": "social_worker",
...
class TestSlotOptimisation:
    ...
    def test_success(self, client):
        token = login(client)
        res = client.post("/api/slot-optimisation", headers=auth(token), json={
            "appointments": [
                {**SAMPLE_PATIENT, "slotMinutes": 15},
                {"Age": 30, "Gender": 1, "AppointmentLeadTimeDays": 3,
                 "SMSReceived": 1, "PriorDNACount": 0, "IMDDecile": 8,
                 "slotMinutes": 15},
            ],
        })
        assert res.status_code == 200
    ...
    def test_empty_rejected(self, client):
        token = login(client)
        res = client.post("/api/slot-optimisation", headers=auth(token),
                          json={"appointments": []})
        assert res.status_code == 400
    ...
    def test_too_many_rejected(self, client):
        token = login(client)
        res = client.post("/api/slot-optimisation", headers=auth(token),
                          json={"appointments": [SAMPLE_PATIENT] * 51})
        assert res.status_code == 400
...
class TestPatientNudge:
    ...
    def test_elderly_gets_transport_nudge(self, client):
        token = login(client)
        res = client.post("/api/patient-nudge", headers=auth(token), json={
            "patient": SAMPLE_PATIENT, "language": "en",
        })
        assert res.status_code == 200
        data = json.loads(res.data)
    ...
    def test_unknown_language_falls_back_to_en(self, client):
        token = login(client)
        res = client.post("/api/patient-nudge", headers=auth(token), json={
            "patient": SAMPLE_PATIENT, "language": "zz",
        })
        assert res.status_code == 200
    ...
    def test_welsh_message_returned(self, client):
        token = login(client)
        res = client.post("/api/patient-nudge", headers=auth(token), json={
            "patient": SAMPLE_PATIENT, "language": "cy",
        })
        data = json.loads(res.data)
    ...
    def test_patient_name_personalisation(self, client):
        token = login(client)
        res = client.post("/api/patient-nudge", headers=auth(token), json={
            "patient": SAMPLE_PATIENT, "patientName": "Margaret",
        })
...
class TestTwoFactor:
    ...
    def test_enable_rejects_bad_code(self, client):
        ...
        res = client.post("/api/profile/2fa/enable", headers=auth(token),
                          json={"code": "000000"})
        assert res.status_code == 400
    ...
    def test_full_cycle_enable_login_disable(self, client, app):
        ...
        enable = client.post("/api/profile/2fa/enable", headers=auth(token),
...
class TestAuditLog:
    ...
    def test_admin_sees_proxy_events(self, client):
        ...
        client.post("/api/carer-proxy", headers=auth(staff), json={
            "carerName": "Jane", "relationship": "family",
            "patientIdentifier": "NHS001",
        })
...
class TestNotifications:
    ...
    def test_schedule_high_risk(self, client):
        token = login(client)
        res = client.post("/api/notifications/schedule", headers=auth(token), json={
            "patient_id": "NHS001", "risk_tier": "High",
    ...
    def test_reject_low_risk(self, client):
        token = login(client)
        res = client.post("/api/notifications/schedule", headers=auth(token), json={
            "patient_id": "NHS001", "risk_tier": "Low",
        })
        assert res.status_code == 400

    def test_list(self, client):
        token = login(client)
        client.post("/api/notifications/schedule", headers=auth(token), json={
            "patient_id": "NHS001", "risk_tier": "Medium",
        })
...
class TestProfile:
    ...
    def test_update_display_name(self, client):
        token = login(client)
        res = client.put("/api/profile", headers=auth(token),
                         json={"displayName": "Asha Patel"})
        assert res.status_code == 200
