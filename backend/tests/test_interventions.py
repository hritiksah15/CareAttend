...
import pytest
from ml.interventions import generate_interventions, INTERVENTION_RULES


class TestInterventionGeneration:
    def test_returns_tuple(self):
        patient = {"Age": 72, "PriorDNACount": 3, "Disability": 0,
                    "IMDDecile": 5, "AppointmentLeadTimeDays": 7, "SMSReceived": 1}
    ...
    def test_high_prior_dna_triggers_phone(self):
        patient = {"Age": 40, "PriorDNACount": 5, "Disability": 0,
                    "IMDDecile": 5, "AppointmentLeadTimeDays": 7, "SMSReceived": 1}
    ...
    def test_elderly_triggers_transport(self):
        patient = {"Age": 70, "PriorDNACount": 0, "Disability": 0,
                    "IMDDecile": 5, "AppointmentLeadTimeDays": 7, "SMSReceived": 1}
        interventions, _, _ = generate_interventions(patient, 0.5, [])
        titles = [iv["title"] for iv in interventions]
    ...
    def test_85_plus_triggers_clinical_triage(self):
        patient = {"Age": 88, "PriorDNACount": 0, "Disability": 0,
                    "IMDDecile": 5, "AppointmentLeadTimeDays": 7, "SMSReceived": 1}
    ...
    def test_disability_triggers_accessibility(self):
        patient = {"Age": 40, "PriorDNACount": 0, "Disability": 1,
                    "IMDDecile": 5, "AppointmentLeadTimeDays": 7, "SMSReceived": 1}
        interventions, _, _ = generate_interventions(patient, 0.5, [])
        titles = [iv["title"] for iv in interventions]
    ...
    def test_deprived_triggers_social_support(self):
        patient = {"Age": 40, "PriorDNACount": 0, "Disability": 0,
                    "IMDDecile": 2, "AppointmentLeadTimeDays": 7, "SMSReceived": 1}
        interventions, _, _ = generate_interventions(patient, 0.5, [])
        titles = [iv["title"] for iv in interventions]
    ...
    def test_no_sms_triggers_followup(self):
        patient = {"Age": 40, "PriorDNACount": 0, "Disability": 0,
                    "IMDDecile": 5, "AppointmentLeadTimeDays": 7, "SMSReceived": 0}
        interventions, _, _ = generate_interventions(patient, 0.5, [])
        titles = [iv["title"] for iv in interventions]
        assert "Enhanced SMS Follow-up" in titles

    def test_max_five_interventions(self):
        patient = {"Age": 90, "PriorDNACount": 10, "Disability": 1,
                    "IMDDecile": 1, "AppointmentLeadTimeDays": 30, "SMSReceived": 0}
